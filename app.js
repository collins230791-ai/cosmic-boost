...
function updateUI() {
  ...
  if (otherName) otherName.placeholder = lang === 'ru' ? 'Введите имя' : 'Enter name';
  const otherBirth = document.getElementById('other-birth');
  if (otherBirth) otherBirth.placeholder = lang === 'ru' ? 'Введите дату рождения' : 'Enter date of birth';
...
async function loadBoostContent() {
  ...
  // Банк текстов, без Claude: смена при заходе бесплатная и не жжёт токены.
}
...
function parseBirthDate(raw) {
  ...
  const m = v.replace(/\s+/g, '').match(/^(\d{1,2})[./-](\d{1,2})[./-](\d{4})$/);
  if (!m) return '';
...
function prepareSynastry() {
  ...
  const otherBirth = parseBirthDate(document.getElementById('other-birth')?.value || '');
